...
# SMTP server configuration (Example for Gmail)
    ...
    smtp_port = 587
    password = "seniordesign1)1"  # You might need to use an app-specific passw>
...
def ADMIN_menu_SETUP():
    ...
    if temp == ADMIN_password:
        ...
        # message3 = Text(app, text="Please retry. The system will turn off now>
        # close_gui()
...
class KeypadWindow:
    def __init__(self, app, title="Enter Passcode", visible = False):
        ...
        self.passcode = ""
        self.correct_pin = None  # This will store the correct PIN set in the s>
...
class SetupWindow:
    def __init__(self, app, keypad_window, title="Setup PIN Code", visible = False):
        self.window = Window(app, title=title,  height = 320, width = 480,visible=False)  # Start with the win>
        #self.window.full_screen = True
        self.passcode = ""
...
def enroll_new_user_face_voice(username):
    ...
    app.warn("Info:", "The GISS will now enroll your face. Please press the button to capture your a picture oof face when you are ready.")
...
def DISARM():
    app.warn("Warning", "System Disarmed")
    disarm_system()
    logout()
"""
   # print("Developing ... REQUIRES FACE ID AND VOICE RECOGNITION")
